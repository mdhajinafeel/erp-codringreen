...
class Dashboard extends MY_Controller
{
	...
	public function export_map_details()
	{
		...
		$originId = intval($this->input->get("oid"));
		$year = intval($this->input->get("year"));

		$exportMapDetails = $this->Dashboard_model->get_export_map_details($originId, $year);
		$returnMapDataArray = array();
		$returnLegendDataArray = array();
		foreach ($exportMapDetails as $r) {
			
			if($originId == 3) {
				$volume = ($r->total_net_weight + 0) . ' MT';
				$rowMapArrayData['mapdata'] = "$r->pod_name##Total Shipment: $r->total_shipment#Total Containers: $r->total_containers###Total Net Weight: $volume";
			} else {
				$volume = ($r->total_net_volume + 0) . ' '. $this->lang->line('volume_unit');
			...
			$rowLegendArrayData['originid'] = $originId;
			if($originId == 3) {
				$rowLegendArrayData['totalvolume'] = $r->total_net_weight + 0 . " MT";
			} else {
				$rowLegendArrayData['totalvolume'] = $r->total_net_volume + 0 . " ".$this->lang->line('volume_unit');
			}
