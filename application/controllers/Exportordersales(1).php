    ...
    public function export_order_list()
    {
        ...
        if (!empty($session)) {
            ...
            foreach ($exportContainers as $r) {
                ...
                if ($this->input->get("originid") == 3) {
                    $actionExport = '<span data-toggle="tooltip" data-placement="top" title="' . $this->lang->line("invoice_history") . '"><button type="button" class="btn icon-btn btn-xs btn-download waves-effect waves-light" data-role="proformainvoice" data-toggle="modal" data-target=".view-modal-data" data-export_id="' . $r->id . '" data-sa_number="' . $r->sa_number . '"><span class="fa fa-history"></span></button></span>
    ...
    public function generate_proforma_invoice()
    {
        try {
            ...
            if (!empty($session)) {
                ...
                if ($originId == 3) {
                    $objExcel = $objExcel->load('./assets/templates/ProForma_Invoice_Template.xlsx');
                } else {
                    if($bankId == 5) {
                ...
                if ($originId == 3) {
                    ...
                    if (count($getExportItemDetails) > 0) {
                        ...
                        foreach ($getExportItemDetails as $exportitem) {
                            ...
                            $totalValue = $totalValue + $exportitem->metric_ton * $exportitem->unit_price;
                        }
                    }

                    $objSheet->setCellValue("D35", "=SUM(D20:D$exportItemsRowCount)");
                    $objSheet->setCellValue("G35", "=SUM(G20:G$exportItemsRowCount)");
                    $objSheet->setCellValue("F35", "=G35/D35");
                    ...
                    $objSheet->setCellValue("C37", "US Dollars " . str_replace("  ", " ", ucwords($convertedNumber)) . " Only");

                    //BANK NAME
                    $objSheet->setCellValue("C42", $getBankDetails[0]->bank_name);
                    $objSheet->setCellValue("C43", $getBankDetails[0]->bank_address);
                    $objSheet->setCellValue("C44", $getBankDetails[0]->swift_code);
                    $objSheet->setCellValue("C45", $getBankDetails[0]->account_number);
                    $objSheet->getStyle("C45")->getNumberFormat()->setFormatCode('0');
                } else {
                    ...
                    $objSheet->setCellValue("G12", $getExportDetails[0]->origin_name);
                    $objSheet->setCellValue("G13", ucwords($getExportDetails[0]->pol));
                    $objSheet->setCellValue("G14", ucwords($getExportDetails[0]->pod));
