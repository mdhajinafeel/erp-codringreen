                ...
                if ($this->input->post('page_type') == 'add') {
                    ...
                    if (count($getCountryCode) == 1) {
                        ...
                        $dataPurchaseContract = array(
                            ...
                            "purchase_allowance" => $circumferenceallowance,
                            "purchase_allowance_length" => $lengthallowance,
                            "unit_of_purchase" => $measuremetsystem,
                        ...
                        if ($insertPurchaseContract > 0) {
                            ...
                            foreach ($priceArrayJson as $price) {
                                $dataContractPrice = array(
                                    "supplier_id" => $insertPurchaseContract,
                                    "minrange_grade1" => $price["minRange"],
                                    "maxrange_grade2" => $price["maxRange"],
                                    "pricerange_grade3" => $price["priceShorts"],
                                    "pricerange_grade_semi" => $price["priceSemi"],
                                    "pricerange_grade_longs" => $price["priceLongs"],
                                    "created_by" => $session['user_id'],
                                    "updated_by" => $session['user_id'],
                                    'is_active' => $contractstatus,
                                );
        ...
        if ($this->input->get('type') == "viewcontract") {
            if (!empty($session)) {
                ...
                $data = array(
                    ...
                    'description' => $getContractDetails[0]->description,
            ...
            if (!empty($session)) {
                ...
                if ($_FILES['filePriceExcel']['size'] > 0) {
                    ...
                    if (!$this->upload->do_upload('filePriceExcel')) {
                        ...
                        if ($arrayCount > 0) {
                            $createArray = array('MIN RANGE', 'MAX RANGE', 'PRICE SHORTS', 'PRICE SEMI', 'PRICE LONGS');
                            $makeArray = array(
                                'MINRANGE' => 'MIN RANGE',
                                'MAXRANGE' => 'MAX RANGE',
                                'PRICESHORTS' => 'PRICE SHORTS',
                                'PRICESEMI' => 'PRICE SEMI',
                                'PRICELONGS' => 'PRICE LONGS'
                            );
                            $SheetDataKey = array();
                            ...
                            if (empty($data)) {
                                ...
                                for ($i = 2; $i <= $arrayCount; $i++) {
                                    ...
                                    $priceShorts = $SheetDataKey['PRICESHORTS'];
                                    $priceSemi = $SheetDataKey['PRICESEMI'];
                                    $priceLongs = $SheetDataKey['PRICELONGS'];
                                    ...
                                    $priceShortsVal = filter_var(trim($allDataInSheet[$i][$priceShorts]), FILTER_SANITIZE_FULL_SPECIAL_CHARS);
                                    $priceSemiVal = filter_var(trim($allDataInSheet[$i][$priceSemi]), FILTER_SANITIZE_FULL_SPECIAL_CHARS);
                                    $priceLongsVal = filter_var(trim($allDataInSheet[$i][$priceLongs]), FILTER_SANITIZE_FULL_SPECIAL_CHARS);
                                    
                                    if($priceShortsVal == null || $priceShortsVal == "") {
                                        $priceShortsVal = 0;
                                    }

                                    if($priceSemiVal == null || $priceSemiVal == "") {
                                        $priceSemiVal = 0;
                                    }

                                    if($priceLongsVal == null || $priceLongsVal == "") {
                                        $priceLongsVal = 0;
                                    }
                                    ...
                                    if ($originId == 3) {
                                        ...
                                        if (($minRangeVal != null && $minRangeVal != "" && $minRangeVal > 0) &&
                                            ...
                                            ($priceShortsVal != null && $priceShortsVal != "" && $priceShortsVal >= 0) && 
                                            ($priceSemiVal != null && $priceSemiVal != "" && $priceSemiVal >= 0) && 
                                            ($priceLongsVal != null && $priceLongsVal != "" && $priceLongsVal >= 0)
                                        ) {
                                            $priceData[] = array(
                                                'minRange' => ($minRangeVal + 0),
                                                'maxRange' => ($maxRangeVal + 0),
                                                'priceShorts' => ($priceShortsVal + 0),
                                                'priceSemi' => ($priceSemiVal + 0),
                                                'priceLongs' => ($priceLongsVal + 0),
                                            );
                                        }
                                    } else {
                                        ...
                                        if (($minRangeVal != null && $minRangeVal != "" && $minRangeVal > 0) &&
                                            ($maxRangeVal != null && $maxRangeVal != "" && $maxRangeVal > 0) &&
                                            ($priceShortsVal != null && $priceShortsVal != "") && 
                                            ($priceSemiVal != null && $priceSemiVal != "") && 
                                            ($priceLongsVal != null && $priceLongsVal != "")
                                        ) {
