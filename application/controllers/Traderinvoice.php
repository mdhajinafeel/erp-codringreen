...
class Traderinvoice extends MY_Controller
{
    ...
    {
        $data["title"] = $this->lang->line("trader_invoice") . " - " . $this->lang->line("sales") .  " | " . $this->Settings_model->site_title();
        ...
        if (!empty($session)) {
            ...
            $data["subview"] = $this->load->view("exportsales/trader_invoice", $data, TRUE);
    ...
    public function export_order_list()
    {
        ...
        if (!empty($session)) {
            ...
            $traderExports = $this->Export_model->fetch_exportid_trader($this->input->get("traderid"));
            
            $exportIds = 0;
            foreach($traderExports as $tradexport) {
                $exportIds = $exportIds . ", " . $tradexport->export_id;
            }

            $exportContainers = $this->Export_model->trader_exports($exportIds);
            ...
            foreach ($exportContainers as $r) {
                ...
                $historyId = 0;
                foreach($traderExports as $trade) {
                    if($r->id == $trade->export_id) {
                ...
                if ($this->input->get("originid") == 3) {
                    ...
                    $actionExport = '<span data-toggle="tooltip" data-placement="top" title="' . $this->lang->line("invoice_history") . '"><button type="button" class="btn icon-btn btn-xs btn-edit waves-effect waves-light" data-role="invoicehistory" data-toggle="modal" data-target=".view-modal-data" data-export_id="' . $r->id . '" data-invoice_id = "'. $historyId .'" data-sa_number="' . $r->sa_number . '"><span class="fa fa-history"></span></button></span> 
                        <span data-toggle="tooltip" data-placement="top" title="' . $this->lang->line("generate_proforma_invoice") . '"><button type="button" class="btn icon-btn btn-xs btn-download waves-effect waves-light" data-role="proformainvoice" data-toggle="modal" data-target=".view-modal-data" data-export_id="' . $r->id . '" data-invoice_id = "'. $historyId .'" data-sa_number="' . $r->sa_number . '"><span class="fa fa-receipt"></span></button></span>';
    ...
    public function dialog_export_order_option()
    {
        ...
        if (!empty($session)) {
            ...
            if ($this->input->post("type") == "viewexport") {
                ...
                $exportOrderDetail = $this->Export_model->get_export_order_details_by_id_for_trader($exportOrderId, $saNumber);
                if (count($exportOrderDetail) == 1) {
                    ...
                    $data = array(
                        ...
                        "originid" => $exportOrderDetail[0]->origin_id,
                        "exportid" => $exportOrderId,
                        "sanumber" => $saNumber,
                        "export_details" => $exportOrderDetail,
                        "product_type_id" => $exportOrderDetail[0]->product_type_id,
                        "measurementsystems" => $this->Master_model->fetch_measurementsystems_by_origin($exportOrderDetail[0]->origin_id, $exportOrderDetail[0]->product_type_id),
                    );
        ...
        try {
            ...
            if (!empty($session)) {
                ...
                if ($originId == 3) {
                    ...
                    //Fetch Invoice
                    $serviceSales = 0;
                    $servicesalesPercentage = 0;
                    $salesAdvance = 0;
                    $salesAdvanceCost = 0;
                    $fetchInvoiceDetails = $this->Export_model->get_details_invoice_history_trader($this->input->post("invoice_id"));
                    if(count($fetchInvoiceDetails) == 1) {
                        $serviceSales = $fetchInvoiceDetails[0]->service_enabled;
                        $servicesalesPercentage = $fetchInvoiceDetails[0]->service_sales_percentage;
                        $salesAdvance = $fetchInvoiceDetails[0]->advance_enabled;
                        $salesAdvanceCost = $fetchInvoiceDetails[0]->advance_cost;
                    }

                    $transferPrice = $this->input->post("transferprice");
                    $transferPriceCost = $this->input->post("transfer_price_cost");
                    ...
                    if (count($containerDataArray) > 0 && $accountingInvoice == 1) {
                        ...
                        foreach ($containerDataArray as $container) {
                            ...
                            if (count($updateContainerNumberDataJson) > 0) {
                                foreach ($updateContainerNumberDataJson as $containerno) {
                                    if ($containerno["containerNumber"] == $container["containerNumber"]) {
                                        $containerUPrice = ($containerno["containerPrice"] - $accountingInvoiceUnitPrice + $transferPriceCost) + 0;
                                        $objSheet->setCellValue("F$exportItemsRowCount", ($containerno["containerPrice"] - $accountingInvoiceUnitPrice + $transferPriceCost) + 0);
                                        break;
                                    } else {
                                        $containerUPrice = ($container["unitPrice"] - $accountingInvoiceUnitPrice) + 0;
                                        $objSheet->setCellValue("F$exportItemsRowCount", ($container["unitPrice"] - $accountingInvoiceUnitPrice + $transferPriceCost) + 0);
                                    }
                                }
                            } else {
                                $containerUPrice = ($container["unitPrice"] - $accountingInvoiceUnitPrice + $transferPriceCost) + 0;
                                $objSheet->setCellValue("F$exportItemsRowCount", ($container["unitPrice"] - $accountingInvoiceUnitPrice + $transferPriceCost) + 0);
                            }
                ...
                if($originId == 3) {
                    $dataExportInvoiceHistory = array(
                        ...
                        'is_active' => 1,
                        'is_trader' => 1,
                        'tp_enabled' => $transferPrice, 
                        'tp_cost' => $transferPriceCost, 
                    );
                } else {
                    $dataExportInvoiceHistory = array(
                    "export_id" => $exportOrderId,
            ...
            if ($this->input->post("type") == "generateinvoice") {
                ...
                if (count($exportOrderDetail) == 1) {
                    ...
                    $data = array(
                        ...
                        "export_details" => $exportOrderDetail,
                        "product_type_id" => $exportOrderDetail[0]->product_type_id,
                        "buyers" => $this->Master_model->fetch_buyers_list_no_traders($exportOrderDetail[0]->origin_id),
                        "sellers" => $this->Master_model->fetch_sellers_list_traders(),
                        "banks" => $this->Master_model->fetch_banks_invoice_list($exportOrderDetail[0]->origin_id),
                        "export_volume" => $this->Export_model->get_export_volume_by_id($exportOrderId, $saNumber, $exportOrderDetail[0]->origin_id),
                        "measurementsystems" => $this->Master_model->fetch_measurementsystems_by_origin($exportOrderDetail[0]->origin_id, $exportOrderDetail[0]->product_type_id),
                        "company_setting" => $this->Settings_model->read_company_setting($exportOrderDetail[0]->origin_id),
                        "containernumbers" => $this->Export_model->fetch_distinct_container_number($exportOrderId),
                        "claimtrackers" => $this->Claimtracker_model->fetch_unclaim_list($exportOrderDetail[0]->origin_id),
                        "salesBuyerData" => $this->Export_model->get_ledger_transaction_details_by_export($exportOrderId),

                    );

                    $this->load->view("exportsales/dialog_view_trader_invoice", $data);
                } else {
                    $Return["redirect"] = false;
                    $Return["result"] = "";
                    $Return["pageheading"] = $this->lang->line("information");
    ...
    public function dialog_invoice_history_option()
    {
        ...
        if (!empty($session)) {
            ...
            if ($this->input->post("type") == "invoicehistory") {
                ...
                //CHECK EXPORT DETAILS
                $invoiceHistoryData = $this->Export_model->fetch_invoice_history_trader($exportOrderId);
                $exportOrderDetail = $this->Export_model->get_export_order_details_by_id_trader($exportOrderId, $saNumber);
                if (count($exportOrderDetail) == 1) {
                    ...
                    $data = array(
                        ...
                        "csrfhash" => $this->security->get_csrf_hash(),
                        "originid" => $exportOrderDetail[0]->id,
                        "exportid" => $exportOrderId,
                        "sanumber" => $saNumber,
                        "invoiceHistory" => $invoiceHistoryData,
                        "export_details" => $exportOrderDetail,
                        "invoiceContainers" => $this->Export_model->fetch_container_invoice_price_by_export_trader($exportOrderId),
                    );

                    $this->load->view("exportsales/dialog_view_trader_history", $data);
                } else {
                    $Return["redirect"] = false;
                    $Return["result"] = "";
                    $Return["pageheading"] = $this->lang->line("information");
        ...
        try {
            ...
            if (!empty($session)) {
                ...
                if ($originId == 3) {
                    ...
                    $updateContainerNumberDataJson = json_decode($this->input->post("updatecontainerprice"), true);
                    $creditnotes = $invoiceData[0]->claim_id;
                    $transerPriceCost = $invoiceData[0]->tp_cost;
                    ...
                    $getExportItemDetails = $this->Export_model->get_export_data_by_export_id_trader($exportOrderId, $originalSANumber, $circAllowance, $lengthAllowance, $circAdjustment, $measurementsystem);
                    ...
                    if (count($containerDataArray) > 0 && $accountingInvoice == 1) {
                        ...
                        foreach ($containerDataArray as $container) {
                            ...
                            if (count($updateContainerNumberDataJson) > 0) {
                                foreach ($updateContainerNumberDataJson as $containerno) {
                                    if ($containerno["containerNumber"] == $container["containerNumber"]) {
                                        $containerUPrice = ($containerno["containerPrice"] - $accountingInvoiceUnitPrice + $transerPriceCost) + 0;
                                        $objSheet->setCellValue("F$exportItemsRowCount", ($containerno["containerPrice"] - $accountingInvoiceUnitPrice + $transerPriceCost) + 0);
                                        break;
                                    } else {
                                        $containerUPrice = ($container["unitPrice"] - $accountingInvoiceUnitPrice) + 0;
                                        $objSheet->setCellValue("F$exportItemsRowCount", ($container["unitPrice"] - $accountingInvoiceUnitPrice + $transerPriceCost) + 0);
                                    }
                                }
                            } else {
                                $containerUPrice = ($container["unitPrice"] - $accountingInvoiceUnitPrice + $transerPriceCost) + 0;
                                $objSheet->setCellValue("F$exportItemsRowCount", ($container["unitPrice"] - $accountingInvoiceUnitPrice + $transerPriceCost) + 0);
                            }
