    ...
    public function generate_warehouse_reports()
    {
        try {
            ...
            if (!empty($session)) {
                ...
                $startDate = str_replace('/', '-', $reportStartDate);
                $endDate = str_replace('/', '-', $reportEndDate);
                
                $reportStartDate = date('Y-m-d', strtotime($startDate));
                $reportEndDate = date('Y-m-d', strtotime($endDate));

                $fetchWarehouseData_SquareBlocks = array();
                $fetchWarehouseData_RoundLogs = array();

                if ($downloadCriteria == 1) {
                    $fetchWarehouseData_SquareBlocks = $this->Reception_model->get_warehouse_report_by_supplier($originId, $supplierId, $inventoryOrder, 1, $reportStatus);
                    $fetchWarehouseData_RoundLogs = $this->Reception_model->get_warehouse_report_by_supplier($originId, $supplierId, $inventoryOrder, 2, $reportStatus);
                } else if ($downloadCriteria == 2) {
                    $fetchWarehouseData_SquareBlocks = $this->Reception_model->get_warehouse_report_by_daterange($originId, $reportStartDate, $reportEndDate, 1, $reportStatus);
                    $fetchWarehouseData_RoundLogs = $this->Reception_model->get_warehouse_report_by_daterange($originId, $reportStartDate, $reportEndDate, 2, $reportStatus);
                } else if ($downloadCriteria == 3) {
                    $fetchWarehouseData_SquareBlocks = $this->Reception_model->get_warehouse_report_by_product($originId, $productId, 1, $reportStatus);
                    $fetchWarehouseData_RoundLogs = $this->Reception_model->get_warehouse_report_by_product($originId, $productId, 2, $reportStatus);
                    ...
                    $fetchWarehouseData_RoundLogs = $this->Reception_model->get_warehouse_report_by_producttype_round_logs($originId, $productTypeId, $reportStatus);
                } else if ($downloadCriteria == 5) {
                    $fetchWarehouseData_SquareBlocks = $this->Reception_model->get_warehouse_report_by_inventory($originId, $inputInventoryOrder, 1, $reportStatus);
                    $fetchWarehouseData_RoundLogs = $this->Reception_model->get_warehouse_report_by_inventory($originId, $inputInventoryOrder, 2, $reportStatus);
                }
                ...
                if (count($fetchWarehouseData_SquareBlocks) > 0 || count($fetchWarehouseData_RoundLogs) > 0) {
                    ...
                    if (count($fetchWarehouseData_RoundLogs) > 0) {
                        ...
                        $objRoundLogs->SetCellValue("Q$dataRowCount", $this->lang->line("status"));
                        $objRoundLogs->SetCellValue("R$dataRowCount", $this->lang->line("dispatched_containers"));
                        $objRoundLogs->SetCellValue("S$dataRowCount", $this->lang->line("remaining_pieces"));

                        $objRoundLogs->getStyle("A$dataRowCount:S$dataRowCount")->getFont()->setBold(true);
                        $objRoundLogs->setAutoFilter("A$dataRowCount:S$dataRowCount");
                        $objRoundLogs->getStyle("A$dataRowCount:S$dataRowCount")->getAlignment()->setHorizontal(PHPExcel_Style_Alignment::HORIZONTAL_CENTER);
                        ...
                        foreach ($fetchWarehouseData_RoundLogs as $warehousedata) {
                            ...
                            if($warehousedata->isdispatch == 1) {
                                $objRoundLogs->SetCellValue("Q$dataRowCount", $this->lang->line("dispatched"));
                            } else {
                                $objRoundLogs->SetCellValue("Q$dataRowCount", $this->lang->line("stock"));
                            }

                            $objRoundLogs->SetCellValue("R$dataRowCount", $warehousedata->container_number);
                            $objRoundLogs->SetCellValue("S$dataRowCount", $warehousedata->remaining_stock_count);
                        }
                        ...
                        $objRoundLogs->getStyle("A$startRowCount:S$dataRowCount")->applyFromArray($styleArray);
                        ...
                        $objRoundLogs->getColumnDimension("Q")->setAutoSize(true);
                        $objRoundLogs->getColumnDimension("R")->setAutoSize(true);
                        $objRoundLogs->getColumnDimension("S")->setAutoSize(true);
