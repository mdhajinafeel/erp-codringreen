...
class Farms extends MY_Controller
{
            ...
            if (
                ...
                if (count($getContractDetails) == 1) {
                    $data = array(
                        ...
                        'length_allowance' => ($getContractDetails[0]->length_allowance + 0),
                        'circumference_allowance' => ($getContractDetails[0]->circumference_allowance + 0),
                    );
    ...
    public function get_shipping_lines_by_origin()
    {
        ...
        if (!empty($session)) {
            $result = "<option value='0'>" . $this->lang->line('select') . "</option>";
            if ($this->input->get('originid') > 0) {
                $getSL = $this->Master_model->get_shippinglines_by_origin($this->input->get('originid'));
                foreach ($getSL as $shippingline) {
                        ...
                        if ($getInventoryOrderCount[0]->cnt == 0) {
                            ...
                            if ($productid == 4 && ($producttypeid == 1 || $producttypeid == 3)) {
                                ...
                                foreach ($farmdataJson as $farm) {
                                    ...
                                    $circumference = $farm["circumference"];
                                    $totalVolume = $totalVolume + $farm["netVolume"];
                                    $totalGrossVolume = $totalGrossVolume + $farm["grossVolume"];
                                    $totalPieces = $totalPieces + $farm["noOfPieces"];
                                    $noOfPieces = $farm["noOfPieces"];
                                    $length = $farm["length"];
                                    ...
                                    if ($purchaseunit == 3 || $purchaseunit == 4 || $purchaseunit == 5 || $purchaseunit == 15) {
                                        ...
                                        if ($noOfPieces > 0) {
                                            $getPriceRanges = $this->Farm_model->get_price_for_circumference_length($circumference, $purchasecontractid, $length);

                                            if (count($getPriceRanges) == 1) {
                                                if ($purchaseunit == 3) {
                                                    $value = round($getPriceRanges[0]->price * $noOfPieces, 3);
                                                } else if ($purchaseunit == 4 || $purchaseunit == 5) {
                                                    $value = round($getPriceRanges[0]->price * $farm["netVolume"], 3);
                                                } else if ($purchaseunit == 15) {
                                                    $value = round($getPriceRanges[0]->price, 3);
                                                }

                                                $woodValue = $woodValue + $value;
                                            }
                                        }
                                    } else if ($purchaseunit == 6 || $purchaseunit == 7 || $purchaseunit == 13) {
                    ...
                    if ($input_inventory_order == $inventory_order) {
                        ...
                        if ($getInventoryOrderCount[0]->cnt == 0) {
                            ...
                            if ($productTypeId == 1 || $productTypeId == 3) {
                                ...
                                //END CALCULATE WOOD VALUE & TAXES
    ...
    public function load_farm_template()
    {
        try {
            ...
            if (!empty($session)) {
                ...
                $productId = $this->input->post("productId");
                $purchaseContractId = $this->input->post("purchaseContractId");
                $lengthAllowanceInput = $this->input->post("length_allowance");
                $circumferenceAllowanceInput = $this->input->post("circumference_allowance");
                $roundingfactorInput = $this->input->post("roundingfactor");
                    ...
                    if ($_FILES['fileFarmExcel']['size'] > 0) {
                        ...
                        if (!$this->upload->do_upload('fileFarmExcel')) {
                            ...
                            if ($arrayCount > 0) {
                                ...
                                $createArray = array('CONTAINER', 'SEAL', 'MT', 'SHORT TONS', 'NET LBS', 'DIAMETER', 'LENGTH', 'PURCHASE PRICE', 'SALES PRICE');
                                $makeArray = array('CONTAINER' => 'CONTAINER', 'SEAL' => 'SEAL', 'MT' => 'MT', 'SHORTTONS' => 'SHORT TONS', 'NETLBS' => 'NET LBS', 'DIAMETER' => 'DIAMETER', 'LENGTH' => 'LENGTH', 'PURCHASEPRICE' => 'PURCHASE PRICE', 'SALESPRICE' => 'SALES PRICE');
                                $SheetDataKey = array();
                                ...
                                if (empty($data)) {
                                    ...
                                    for ($i = 2; $i <= $arrayCount; $i++) {
                                        ...
                                        $seal = $SheetDataKey['SEAL'];
                                        $metricTon = $SheetDataKey['MT'];
                                        $shortTons = $SheetDataKey['SHORTTONS'];
                                        $netLbs = $SheetDataKey['NETLBS'];
                                        $diameter = $SheetDataKey['DIAMETER'];
                                        $length = $SheetDataKey['LENGTH'];
                                        $purchasePrice = $SheetDataKey['PURCHASEPRICE'];
                                        $salesPrice = $SheetDataKey['SALESPRICE'];

                                        $containerVal = filter_var(trim($allDataInSheet[$i][$container]), FILTER_SANITIZE_FULL_SPECIAL_CHARS);
                                        $sealVal = filter_var(trim($allDataInSheet[$i][$seal]), FILTER_SANITIZE_FULL_SPECIAL_CHARS);
                                        $metricTonVal = filter_var(trim($allDataInSheet[$i][$metricTon]), FILTER_SANITIZE_FULL_SPECIAL_CHARS);
                                        $shortTonsVal = filter_var(trim($allDataInSheet[$i][$shortTons]), FILTER_SANITIZE_FULL_SPECIAL_CHARS);
                                        $netLbsVal = filter_var(trim($allDataInSheet[$i][$netLbs]), FILTER_SANITIZE_FULL_SPECIAL_CHARS);
                                        $diameterVal = filter_var(trim($allDataInSheet[$i][$diameter]), FILTER_SANITIZE_FULL_SPECIAL_CHARS);
                                        $lengthVal = filter_var(trim($allDataInSheet[$i][$length]), FILTER_SANITIZE_FULL_SPECIAL_CHARS);
                                        $purchasePriceVal = filter_var(trim($allDataInSheet[$i][$purchasePrice]), FILTER_SANITIZE_FULL_SPECIAL_CHARS);
                                        $salesPriceVal = filter_var(trim($allDataInSheet[$i][$salesPrice]), FILTER_SANITIZE_FULL_SPECIAL_CHARS);

                                        if (($containerVal != null && $containerVal != "") &&
                                            ($sealVal != null && $sealVal != "") &&
                                            ($metricTonVal != null && $metricTonVal != "") &&
                                            ($shortTonsVal != null && $shortTonsVal != "") &&
                                            ($netLbsVal != null && $netLbsVal != "") &&
                                            ($diameterVal != null && $diameterVal != "") &&
                                            ($lengthVal != null && $lengthVal != "")
                                        ) {

                                            if ($objPHPExcel->sheetNameExists($containerVal)) {
                                                $objPHPExcel->setActiveSheetIndexByName($containerVal);
                                                $allDataInSheetContainer = $objPHPExcel->getActiveSheet()->toArray(null, true, true, true);
                                                $arrayCountContainerData = count($allDataInSheetContainer);

                                                if ($arrayCountContainerData > 0) {

                                                    $createArrayContainer = array('PIECES', 'DIAMETER', 'LENGTH');
                                                    $makeArrayContainer = array('PIECES' => 'PIECES', 'DIAMETER' => 'DIAMETER', 'LENGTH' => 'LENGTH');
                                                    $SheetDataKeyContainer = array();
                                                    foreach ($allDataInSheetContainer as $dataInSheet) {
                                                        foreach ($dataInSheet as $key => $value) {

                                                            if (in_array(trim($value), $createArrayContainer)) {
                                                                $value = preg_replace('/\s+/', '', $value);
                                                                $SheetDataKeyContainer[trim($value)] = $key;
                                                            }
                                                        }
                                                    }

                                                    $dataContainer = array_diff_key($makeArrayContainer, $SheetDataKeyContainer);

                                                    if (empty($dataContainer)) {

                                                        $farmContainerData = array();
                                                        $totalContainerVolume = 0;
                                                        $totalDiamater = 0;

                                                        for ($j = 2; $j <= $arrayCountContainerData; $j++) {

                                                            $pieces = $SheetDataKeyContainer['PIECES'];
                                                            $diameter = $SheetDataKeyContainer['DIAMETER'];
                                                            $length = $SheetDataKeyContainer['LENGTH'];

                                                            $piecesContainerVal = filter_var(trim($allDataInSheetContainer[$j][$pieces]), FILTER_SANITIZE_FULL_SPECIAL_CHARS);
                                                            $diameterContainerVal = filter_var(trim($allDataInSheetContainer[$j][$diameter]), FILTER_SANITIZE_FULL_SPECIAL_CHARS);
                                                            $lengthContainerVal = filter_var(trim($allDataInSheetContainer[$j][$length]), FILTER_SANITIZE_FULL_SPECIAL_CHARS);

                                                            $lengthContainerVal1 = 0;

                                                            if ($lengthContainerVal >= 11 && $lengthContainerVal < 12) {
                                                                $lengthContainerVal1 = 11;
                                                            } else if ($lengthContainerVal >= 5 && $lengthContainerVal < 6) {
                                                                $lengthContainerVal1 = 5;
                                                            }

                                                            if (($piecesContainerVal != null && $piecesContainerVal != "" && $piecesContainerVal > 0) &&
                                                                ($diameterContainerVal != null && $diameterContainerVal != "" && $diameterContainerVal > 0) &&
                                                                ($lengthContainerVal != null && $lengthContainerVal != "" && $lengthContainerVal > 0)
                                                            ) {

                                                                $firstValue = (($diameterContainerVal + ($lengthContainerVal1 - 4) / 2) * ($diameterContainerVal + ($lengthContainerVal1 - 4) / 2));
                                                                $secondValue = ($lengthContainerVal / 10000);

                                                                $jasVolume = $firstValue * $secondValue;
                                                                $totalContainerVolume = $totalContainerVolume + $jasVolume;
                                                                $totalDiamater = $totalDiamater + $diameterContainerVal;

                                                                $farmContainerData[] = array(
                                                                    'pieces' => $piecesContainerVal + 0,
                                                                    'diameter' => sprintf('%0.3f', $diameterContainerVal),
                                                                    'length' => sprintf('%0.3f', $lengthContainerVal),
                                                                    'volume' => sprintf('%0.3f', $jasVolume),
                                                                );
                                                            }
                                                        }
                                                    }
                                                ...
                                                $totalPieces = $totalPieces + count($farmContainerData);
                                                $totalMT = $totalMT + $metricTonVal;
                                                $totalST = $totalST + $shortTonsVal;
                                                $totalNetLbs = $totalNetLbs + $netLbsVal;
                                                $totalNetVolume = $totalNetVolume + $totalContainerVolume;
                                                $averageDiameter = $totalDiamater / count($farmContainerData);
                                                ...
                                                if ($purchasePriceVal != 0) {
                                                    ...
                                                    preg_match_all('/\d*\.?\d+/', $lengthVal, $matches1);
                                                    $diameterNumbersVal = $matches[0];
                                                    $lengthNumbersVal = $matches1[0];

                                                    if ($purchaseUnit == 10 || $purchaseUnit == 11) {
                                                        $getPriceRanges = $this->Farm_model->get_price_for_diameter_length($diameterNumbersVal[0], $lengthNumbersVal[0], $purchaseContractId);
                                                        if (count($getPriceRanges) > 0) {
                                                            $woodValue = $getPriceRanges[0]->pricerange_grade3;
                                                            $totalPurchaseValue = $getPriceRanges[0]->pricerange_grade3 * $metricTonVal;
                                                        }
                                                    }
                                                }
                                                ...
                                                $farmData[] = array(
                                                    ...
                                                    'seal' => $sealVal,
                                                    'metricTon' => sprintf('%0.13f', $metricTonVal),
                                                    'shortTons' => sprintf('%0.3f', $shortTonsVal),
                                                    'netLbs' => sprintf('%0.3f', $netLbsVal),
                                                    'diameter' => $diameterVal,
                                                    'length' => $lengthVal,
                                                    'totalCount' => count($farmContainerData),
                                                    'averageDiameter' => sprintf('%0f', $averageDiameter),
                                                    'jasVolume' => sprintf('%0.3f', $totalContainerVolume),
                                                    'purchasePrice' => sprintf('%0.2f', $woodValue),
                                                    'totalPurchasePrice' => sprintf('%0.2f', $totalPurchaseValue),
                                                    'salesPrice' => sprintf('%0.2f', $salesValue),
                                                    'totalSalesPrice' => sprintf('%0.2f', $totalSalesValue),
                                                    'containerData' => $farmContainerData,
                                                );
                                            } else {
                                                ...
                                                $totalST = $totalST + $shortTonsVal;
                                                $totalNetLbs = $totalNetLbs + $netLbsVal;
                                                $totalNetVolume = $totalNetVolume + 0;
                                                ...
                                                $farmData[] = array(
                                                    ...
                                                    'averageDiameter' => 0,
                                                    'jasVolume' => 0,
                                                    'purchasePrice' => sprintf('%0.2f', 0),
                                                    'totalPurchasePrice' => sprintf('%0.2f', 0),
                                                    'salesPrice' => sprintf('%0.2f', 0),
                                                    'totalSalesPrice' => sprintf('%0.2f', 0),
                                                    'containerData' => array(),
                                                );
                                            }
                                        }
                                    ...
                                    if (count($farmData) == 0) {
                                        ...
                                        $dataUploaded = array(
                                            ...
                                            "totalMT" => sprintf('%0.13f', $totalMT),
                                            "totalST" => sprintf('%0.3f', $totalST),
                                            "totalNetLbs" => sprintf('%0.3f', $totalNetLbs),
                                            "purchaseUnit" => $purchaseUnit,
                                            "totalVolume" => sprintf('%0.3f', $totalNetVolume),
                                            "totalGrossVolume" => sprintf('%0.3f', $totalNetVolume),
                                        );
                        ...
                        if ($_FILES['fileFarmExcel']['size'] > 0) {
                            ...
                            if (!$this->upload->do_upload('fileFarmExcel')) {
                                ...
                                if ($arrayCount > 0) {
                                    ...
                                    $circumferenceAllowance = $circumferenceAllowanceInput;
                                    $lengthAllowance = $lengthAllowanceInput;
                                        ...
                                        if (empty($data)) {
                                            ...
                                            for ($i = 2; $i <= $arrayCount; $i++) {
                                                ...
                                                if (($inventoryOrderVal != null && $inventoryOrderVal != "") &&
                                                    ($scannedCode != null && $scannedCode != "" && $scannedCode > 0) &&
                                                    ...
                                                    if ($inventoryOrderVal == $inventoryOrderPost) {
                                                        ...
                                                        $totalPieces = $totalPieces + $scannedCode;
                                                    }
                                                }
                                            }
            ...
            if (!empty($session)) {
                ...
                if (count($getFarmDetails) == 1 && count($getFarmDataDetails) > 0) {
                    ...
                    if (
                        ...
                        $objSheet->SetCellValue('J' . $rowCount, $this->lang->line('face'));
                        ...
                        $getInventoryContractPrice = $this->Financemaster_model->get_contract_price_data($contractId, $inventoryOrder);

                        if (count($getInventoryContractPrice) > 0) {


                            $objSheet->SetCellValue("N10", $this->lang->line("face"));
                            $objSheet->mergeCells("N10:O10");
                            $objSheet->SetCellValue("P10", $this->lang->line("volume_per_piece"));

                            $objSheet->getStyle("N10:P10")->getFill()->setFillType(PHPExcel_Style_Fill::FILL_SOLID)->getStartColor()->setRGB("DBEDFF");
                            $objSheet->getStyle("N10:P10")->getFont()->setBold(true);
                            $objSheet->getStyle("N10:P10")->getAlignment()->setHorizontal(PHPExcel_Style_Alignment::HORIZONTAL_CENTER);
                            $objSheet->getStyle("N10:P10")->applyFromArray($styleArray);

                            $priceSummaryRow = 11;
                            foreach ($getInventoryContractPrice as $pricedata) {

                                $objSheet->SetCellValue("N$priceSummaryRow", ($pricedata->minrange_grade1 + 0));
                                $objSheet->SetCellValue("O$priceSummaryRow", ($pricedata->maxrange_grade2 + 0));

                                if ($getFarmDetails[0]->exchange_rate > 0) {
                                    $priceRange = $pricedata->pricerange_grade3 * $getFarmDetails[0]->exchange_rate;
                                } else {
                                    $priceRange = $pricedata->pricerange_grade3;
                                }

                                $objSheet->SetCellValue("P$priceSummaryRow", ($priceRange + 0));
                                //$objSheet->getStyle("P$priceSummaryRow:P$priceSummaryRow")->getNumberFormat()->setFormatCode($getCurrency[0]->currency_excel_format);

                                $objSheet->getStyle("P$priceSummaryRow")
                                    ->getNumberFormat()
                            ...
                            $objSheet->getStyle("N10:P$priceLastRow")->applyFromArray($styleArray);
                        }

                        $objSheet->getColumnDimension("N")->setAutoSize(false)->setWidth(10);
                        $objSheet->getColumnDimension("O")->setAutoSize(false)->setWidth(10);
                        ...
                        if ($getFarmDetails[0]->unit_of_purchase == 8 || $getFarmDetails[0]->unit_of_purchase == 9 || $getFarmDetails[0]->unit_of_purchase == 12 || $getFarmDetails[0]->unit_of_purchase == 14) {
                        ...
                        if (count($getFarmDataSummary) > 0) {
                            ...
                            if ($getFarmDetails[0]->unit_of_purchase == 4 || $getFarmDetails[0]->unit_of_purchase == 5 || $getFarmDetails[0]->unit_of_purchase == 9 || $getFarmDetails[0]->unit_of_purchase == 12 || $getFarmDetails[0]->unit_of_purchase == 14) {
                            ...
                            } else if ($getFarmDetails[0]->unit_of_purchase == 6 || $getFarmDetails[0]->unit_of_purchase == 7 || $getFarmDetails[0]->unit_of_purchase == 13) {
                                $objSheet->SetCellValue("H$rowCountSummary", $this->lang->line('volume'));
                                $objSheet->SetCellValue("I$rowCountSummary", $this->lang->line('value_volume'));
                            } else {
                            ...
                            foreach ($getFarmDataSummary as $farmdatasummary) {
                                ...
                                if (count($return_arr_farmdata) > 0) {
                                    ...
                                    } else if ($getFarmDetails[0]->unit_of_purchase == 9 || $getFarmDetails[0]->unit_of_purchase == 12 || $getFarmDetails[0]->unit_of_purchase == 14) {
                                        $objSheet->SetCellValue("H$rowCountDataSummary", "=IF(" . '$D$' . "$summaryHeaderLastRow" . ">=F$rowCountDataSummary,IF(" . '$D$' . "$summaryHeaderLastRow" . "<=G$rowCountDataSummary," . '$D$' . "3,0),0)");
                                    } else {
            ...
            if (!empty($session)) {
                ...
                if (count($getFarmDetails) == 1 && count($getFarmDataDetails) > 0) {
                    ...
                    if (
                        ...
                        $objSheet->SetCellValue("C$rowCount", $this->lang->line('pieces'));
                        $objSheet->SetCellValue("D$rowCount", $this->lang->line('volume'));
                        $objSheet->SetCellValue("E$rowCount", $this->lang->line('product_type'));

                        $objSheet->getStyle("A$rowCount:E$rowCount")->getFont()->setBold(true);
                        $objSheet->setAutoFilter("A$rowCount:E$rowCount");

                        $objSheet->getStyle("A$rowCount:E$rowCount")->getAlignment()->setHorizontal(PHPExcel_Style_Alignment::HORIZONTAL_CENTER);
                        ...
                        $objSheet->getStyle("A$rowCount:E$rowCount")->getFill()->setFillType(PHPExcel_Style_Fill::FILL_SOLID)->getStartColor()->setRGB('add8e6');
                        $objSheet->getStyle("A$rowCount:E$rowCount")->applyFromArray($styleArray);

                        $calcStartRow = $rowCount + 1;
                        $rowCountData = $rowCount + 1;
                        $farmDataFirstRow = $rowCountData;
                        $grossVolumeTotal = 0;
                        foreach ($getFarmDataDetails as $farmdata) {
                            $objSheet->SetCellValue('A' . $rowCountData, ($farmdata->circumference + 0));
                            $objSheet->SetCellValue('B' . $rowCountData, ($farmdata->length + 0));
                            ...
                            $objSheet->SetCellValue("E$rowCountData", "=IF(B$rowCountData<330, $textShorts ,IF(B$rowCountData>=600, $textLongs, $textSemi))");
                            $objSheet->getStyle("D$rowCountData")->getNumberFormat()->setFormatCode('_(* #,##0.000_);_(* (#,##0.000);_(* "-"??_);_(@_)');
                        ...
                        $lastRowFarmData = $rowCountData - 1;
                        $calcLastRow = $rowCountData - 1;
                        $objSheet->SetCellValue("D2", "=SUM(C$calcStartRow:C$calcLastRow)");
                        ...
                        if (count($getFarmDataSummary) > 0) {
                            ...
                            $objSheet->SetCellValue("G$rowCountSummary", $this->lang->line('circumference_range'));
                            ...
                            if (
                                ...
                                $objSheet->SetCellValue("I$rowCountSummary", $this->lang->line("text_shorts"));
                                $objSheet->SetCellValue("J$rowCountSummary", $this->lang->line("text_semi"));
                                $objSheet->SetCellValue("K$rowCountSummary", $this->lang->line("text_longs"));

                                $objSheet->SetCellValue("L$rowCountSummary", $this->lang->line("farm_total_value"));

                                $objSheet->SetCellValue("M$rowCountSummary", $this->lang->line("text_shorts"));
                                $objSheet->SetCellValue("N$rowCountSummary", $this->lang->line("text_semi"));
                                $objSheet->SetCellValue("O$rowCountSummary", $this->lang->line("text_longs"));

                                $objSheet->getStyle("G$rowCountSummary:O$rowCountSummary")->getFill()->setFillType(PHPExcel_Style_Fill::FILL_SOLID)->getStartColor()->setRGB('DBEDFF');
                            ...
                            foreach ($getFarmDataSummary as $farmdatasummary) {
                                ...
                                $objSheet->SetCellValue("G$rowCountDataSummary", ($farmdatasummary->minrange_grade1 + 0));
                                $objSheet->SetCellValue("H$rowCountDataSummary", ($farmdatasummary->maxrange_grade2 + 0));

                                $priceRangeShorts = $farmdatasummary->pricerange_grade3;
                            ...
                            $objSheet->getColumnDimension('K')->setWidth('18');
                ...
                if (count($getFarmDetails) == 1 && count($getFarmDataDetails) > 0) {
                    ...
                    if ($getFarmDetails[0]->unit_of_purchase == 3 || $getFarmDetails[0]->unit_of_purchase == 4 || $getFarmDetails[0]->unit_of_purchase == 5) {
                        ...
                        if (count($getFarmDataSummary) > 0) {
                            ...
                            if ($getFarmDetails[0]->unit_of_purchase == 4 || $getFarmDetails[0]->unit_of_purchase == 5 || $getFarmDetails[0]->unit_of_purchase == 6 || $getFarmDetails[0]->unit_of_purchase == 7 || $getFarmDetails[0]->unit_of_purchase == 9 || $getFarmDetails[0]->unit_of_purchase == 12 || $getFarmDetails[0]->unit_of_purchase == 13 || $getFarmDetails[0]->unit_of_purchase == 14) {
            ...
            if (!empty($session)) {
                ...
                if (count($getFarmDetails) == 1 && count($getFarmDataDetails) > 0) {
                    ...
                    if (
                        $getFarmDetails[0]->unit_of_purchase == 3 || $getFarmDetails[0]->unit_of_purchase == 4 || $getFarmDetails[0]->unit_of_purchase == 5
                        || $getFarmDetails[0]->unit_of_purchase == 6 || $getFarmDetails[0]->unit_of_purchase == 7
                        || $getFarmDetails[0]->unit_of_purchase == 8 || $getFarmDetails[0]->unit_of_purchase == 9 || $getFarmDetails[0]->unit_of_purchase == 15
                    ) {
                        ...
                        $objSheet->SetCellValue("D$rowCount", $this->lang->line('logistic_cost'));
                        $objSheet->SetCellValue("E$rowCount", $getFarmDetails[0]->logistic_cost);
                        $objSheet->getStyle("E$rowCount")->getNumberFormat()->setFormatCode($getFarmDetails[0]->currency_excel_format);
                        $logisticCostRow = "$rowCount";
                        $rowCount++;
                        ...
                        $fetchContractPrice = $this->Farm_model->get_farm_data_summary($farmId, $contractId, $inventoryOrder, $getFarmDetails[0]->product_type_id);
                        ...
                        if (count($getFarmDataSummary) > 0) {
                            ...
                            if (
                                $getFarmDetails[0]->unit_of_purchase == 4 || $getFarmDetails[0]->unit_of_purchase == 5 || $getFarmDetails[0]->unit_of_purchase == 9
                                || $getFarmDetails[0]->unit_of_purchase == 12 || $getFarmDetails[0]->unit_of_purchase == 14
                            ) {
                                $objSheet->SetCellValue("C$rowCountSummary", $this->lang->line('volume'));
                                $objSheet->SetCellValue("D$rowCountSummary", $this->lang->line('value_volume'));
                            } else if ($getFarmDetails[0]->unit_of_purchase == 6 || $getFarmDetails[0]->unit_of_purchase == 7 || $getFarmDetails[0]->unit_of_purchase == 13) {
                            ...
                            $objSheet->getStyle("A$rowCountSummary:E$rowCountSummary")->getFill()->setFillType(PHPExcel_Style_Fill::FILL_SOLID)->getStartColor()->setRGB('DBEDFF');
                            $objSheet->getStyle("A$rowCountSummary:E$rowCountSummary")->getFont()->setBold(true);
                            $objSheet->getStyle("A$rowCountSummary:E$rowCountSummary")->getAlignment()->setHorizontal(PHPExcel_Style_Alignment::HORIZONTAL_CENTER);
                            $objSheet->getStyle("A$rowCountSummary:E$rowCountSummary")->applyFromArray($styleArray);
