...
class Suppliers extends MY_Controller
{
		...
		if ($this->input->post('add_type') == 'suppliers') {
			...
			if (!empty($session)) {
				...
				if ($this->input->post('action_type') == 'add') {
					...
					$companycode = "CGR";
					$generatealpha = $this->supplierCodeSequence($supplier_origin);
					...
					if ($updateSupplier == true) {
						...
						if ($supplier_role_enabled == 1) {
							...
							if ($insertSupplierRoles > 0) {
								...
								$updateSupplierTaxes = array(
									"is_active" => 0,
									"updated_by" => $session['user_id'],
								);

								$this->Master_model->delete_supplier_taxes($supplier_id, $updateSupplierTaxes);
								...
								foreach ($supplierTaxes as $suppliertax) {
									...
									if ($suppliertax["taxenabled"] == true) {
										$dataSupplierTaxes[] = array(
											"supplier_id" => $supplier_id, "tax_id" => $suppliertax["taxid"],
											"tax_value" => $suppliertax["taxvalue"], "created_by" => $session['user_id'],
											"updated_by" => $session['user_id'], "is_active" => 1,
											"created_date" => date('Y-m-d H:i:s'), "updated_date" => date('Y-m-d H:i:s')
										);
						...
						if ($provider_role_enabled == 1) {
							...
							if ($insertSupplierRoles > 0) {
								...
								$updateProviderTaxes = array(
									"is_active" => 0,
									"updated_by" => $session['user_id'],
								);

								$this->Master_model->delete_provider_taxes($supplier_id, $updateProviderTaxes);
								...
								foreach ($providerTaxes as $providertax) {
									...
									if ($providertax["taxenabled"] == true) {
										$dataProviderTaxes[] = array(
											"supplier_id" => $supplier_id, "tax_id" => $providertax["taxid"],
											"tax_value" => $providertax["taxvalue"], "created_by" => $session['user_id'],
											"updated_by" => $session['user_id'], "is_active" => 1,
											"created_date" => date('Y-m-d H:i:s'), "updated_date" => date('Y-m-d H:i:s')
										);
	...
	public function dialog_supplier_view()
	{
		...
		if ($this->input->get('type') == "viewsupplier") {
			if (!empty($session)) {
				...
				$getOriginSupplierTaxes = $this->Master_model->get_supplier_taxes_by_origin($getSupplierDetails[0]->origin_id);
				$getOriginProviderTaxes = $this->Master_model->get_provider_taxes_by_origin($getSupplierDetails[0]->origin_id);

				$bankArray = array();
				if($getSupplierDetails[0]->banks != "") {
					$bankArray = $getSupplierDetails[0]->banks = explode('-----', $getSupplierDetails[0]->banks);
				}
				...
				$data = array(
					...
					'origin_supplier_taxes' => $getOriginSupplierTaxes,
					'origin_provider_taxes' => $getOriginProviderTaxes,
				);
	...
	public function supplier_taxes_origin()
	{
		...
		if (!empty($session)) {
			...
			foreach ($getSupplierTaxes as $suppliertax) {
				...
				$supplierTaxes = $supplierTaxes . '</div>';
				$supplierTaxes = $supplierTaxes . '<div class="input-group">';
				$supplierTaxes = $supplierTaxes . '<input value="'.($suppliertax->default_tax_value_supplier + 0).'" name="supplier' . $suppliertax->id . '" type="number" step="any" maxlength="8" id="supplier' . $suppliertax->id . '" autocomplete="off" class="form-control" disabled />';
				$supplierTaxes = $supplierTaxes . '</div>';
				$supplierTaxes = $supplierTaxes . '<label id="error-supplier' . $suppliertax->id . '" class="error-text">' . $this->lang->line("error_value") . '</label>';
				$supplierTaxes = $supplierTaxes . '</div>';
				$supplierTaxes = $supplierTaxes . '</div>';
				...
				$supplierTaxes = $supplierTaxes . '$("#checksupplier' . $suppliertax->id . '").change(function() {';
				$supplierTaxes = $supplierTaxes . 'if (this.checked) {';
				$supplierTaxes = $supplierTaxes . '$("#supplier' . $suppliertax->id . '").removeAttr("disabled");';
				$supplierTaxes = $supplierTaxes . '$("#supplier' . $suppliertax->id . '").val("'.($suppliertax->default_tax_value_supplier + 0).'");';
				$supplierTaxes = $supplierTaxes . '} else {';
				$supplierTaxes = $supplierTaxes . '$("#supplier' . $suppliertax->id . '").attr("disabled", "disabled");';
				$supplierTaxes = $supplierTaxes . '$("#supplier' . $suppliertax->id . '").val("'.($suppliertax->default_tax_value_supplier + 0).'");';
				$supplierTaxes = $supplierTaxes . '}';
			...
			foreach ($getProviderTaxes as $providertax) {
				...
				$providerTaxes = $providerTaxes . '</div>';
				$providerTaxes = $providerTaxes . '<div class="input-group">';
				$providerTaxes = $providerTaxes . '<input value="'.($providertax->default_tax_value_provider + 0).'" name="provider' . $providertax->id . '" type="number" step="any" maxlength="8" id="provider' . $providertax->id . '" autocomplete="off" class="form-control" disabled />';
				$providerTaxes = $providerTaxes . '</div>';
				$providerTaxes = $providerTaxes . '<label id="error-provider' . $providertax->id . '" class="error-text">' . $this->lang->line("error_value") . '</label>';
				$providerTaxes = $providerTaxes . '</div>';
				$providerTaxes = $providerTaxes . '</div>';
				...
				$providerTaxes = $providerTaxes . '$("#checkprovider' . $providertax->id . '").change(function() {';
				$providerTaxes = $providerTaxes . 'if (this.checked) {';
				$providerTaxes = $providerTaxes . '$("#provider' . $providertax->id . '").removeAttr("disabled");';
				$providerTaxes = $providerTaxes . '$("#provider' . $providertax->id . '").val("'.($providertax->default_tax_value_provider + 0).'");';
				$providerTaxes = $providerTaxes . '} else {';
				$providerTaxes = $providerTaxes . '$("#provider' . $providertax->id . '").attr("disabled", "disabled");';
				$providerTaxes = $providerTaxes . '$("#provider' . $providertax->id . '").val("'.($providertax->default_tax_value_provider + 0).'");';
				$providerTaxes = $providerTaxes . '}';
	...
	private function supplierCodeSequence($supplier_origin)
	{
		$supplier_record_count = $this->Master_model->supplier_record_count($supplier_origin);
		$result = '';
