...
<div class="card mb-3">
    <div class="card-header table-responsive">
        <div class="row flex-between-end">
            <div class="col-auto align-self-center">
                ...
                <h3> <?php echo $this->lang->line('supplier_credit_header'); ?> </h3>
            </div>
        </div>
    </div>
    <div class="card-body pt-5">
        <div class="mb-3 row">
            ...
            <label class="col-sm-2 col-form-label lbl-font" for="purchase_contract"><?php echo $this->lang->line('purchase_contract'); ?></label>
            <div class="col-sm-10">
                <select class="form-control" name="purchase_contract" id="purchase_contract" data-plugin="select_erp">
                    <option value="0"><?php echo $this->lang->line('select'); ?></option>
                </select>
                <label id="error-purchase-contract" class="error-text"><?php echo $this->lang->line("error_purchase_contract"); ?></label>
                <div class="mb-4 row"></div>
            </div>

            <label class="col-sm-2 col-form-label lbl-font" for="supplier_name"><?php echo $this->lang->line('supplier_name'); ?></label>
            <div class="col-sm-10">
...
<script type="text/javascript">
    ...
    $(document).ready(function() {
        ...
        $("#origin").change(function() {
            fetchPurchaseContracts($("#origin").val());
            fetchSuppliers($("#origin").val(), $("#purchase_contract").val());
        ...
        $("#btn_new_registry").click(function() {
            ...
            var purchase_contract = $("#purchase_contract").val();
            ...
            if (purchase_contract == 0) {
                $("#purchase-contract").show();
                isValid6 = false;
            } else {
                $("#purchase-contract").hide();
                isValid6 = true;
            }
            ...
            if (isValid1 && isValid2 && isValid3 && isValid4 && isValid5) {
                ...
                fd.append("purchase_contract", purchase_contract);
                ...
                $.ajax({
                    type: "POST",
                    url: base_url + "/save_supplier_credit",
                    data: fd,
                    contentType: false,
                    cache: false,
                    processData: false,
                    success: function(JSON) {
                        ...
                        if (JSON.redirect == true) {
                            ...
                            $("#supplier_name option:selected").attr("selectedIndex", 0);
                            $("#amount").val("");
    ...
    function fetchPurchaseContracts(originid) {
        $("#loading").show();
        $.ajax({
            url: base_url + "/get_purchase_contracts_by_origin?originid=" + originid,
            cache: false,
            method: "GET",
            dataType: "json",
            success: function(JSON) {
    ...
    function fetchSuppliers(originid, contractid) {
        $("#loading").show();
        $.ajax({
            url: base_url + "/get_suppliers_by_contract_origin?originid=" + originid + "&contractid=" + contractid,
            cache: false,
            method: "GET",
            dataType: "json",
            success: function(JSON) {
