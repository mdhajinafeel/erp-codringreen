...
<div class="modal-body">
    ...
    <input type="hidden" id="hdnInvoiceId" name="hdnInvoiceId" value="<?php echo $invoiceid; ?>">
    ...
    <?php if ($originid == 1 || $originid == 2 || $originid == 4) { ?>
    ...
    <?php if ($originid == 1 || $originid == 2 || $originid == 4) { ?>
        ...
        <div class="row mb-2">
            <div class="col-md-3 mb-2">
                <input class="form-check-input" id="transferprice" name="transferprice" type="checkbox" value="1">
                <label for="transferprice"><?php echo $this->lang->line('transfer_price'); ?></label>

                <input type="number" id="transfer_price" name="transfer_price" class="form-control text-uppercase" step="1" min="0" oninput="validity.valid||(value='');" disabled />
            </div>
    ...
    <?php if (count($salesBuyerData) > 0) { ?>

        <div class="row mb-2">
            <div class="col-md-3 mb-2">
                <input class="form-check-input" id="update_buyer_ledger" name="update_buyer_ledger" type="checkbox" value="1">
                <label for="update_buyer_ledger"><?php echo $this->lang->line('update_buyer_ledger'); ?></label>
            </div>
        </div>
    ...
    <?php } else { ?>
        ...
        <div class="row mb-2" style="display: none;">
            <div class="col-md-3 mb-2">
                <input class="form-check-input" id="update_buyer_ledger" name="update_buyer_ledger" type="checkbox" value="1" checked>
                <label for="update_buyer_ledger"><?php echo $this->lang->line('update_buyer_ledger'); ?></label>
            </div>
        </div>
    ...
    <div class="row mb-2">
        <div class="col-md-4 mb-2">
            <label for="seller_name"><?php echo $this->lang->line("seller_name"); ?></label>
            <select class="form-control" name="seller_name" id="seller_name" data-plugin="select_erp">
                <option value="0"><?php echo $this->lang->line("select"); ?></option>
                <?php foreach ($sellers as $seller) { ?>
                    <option value="<?php echo $seller->id; ?>"><?php echo $seller->seller_name; ?></option>
                <?php } ?>
            </select>
            <label id="error-sellername" class="error-text"><?php echo $this->lang->line("error_select_name"); ?></label>

            <div class="input-group mt-2">
                <textarea name="seller_name_details" id="seller_name_details" style="font-size: 12px !important;" maxlength="400" rows="8" class="form-control" disabled readonly></textarea>
            </div>
        </div>
        ...
        <div class="col-md-4 mb-2">
            ...
            <div class="input-group mt-2">
                <textarea name="buyer_name_details" id="buyer_name_details" style="font-size: 12px !important;" maxlength="400" rows="8" class="form-control" disabled readonly></textarea>
            </div>
        </div>
        ...
        <div class="col-md-4 mb-2">
            ...
            <div class="input-group mt-2">
                <textarea name="bank_name_details" id="bank_name_details" style="font-size: 12px !important;" maxlength="400" rows="8" class="form-control" disabled readonly></textarea>
            </div>
        </div>
    </div>
</div>
...
<script type="text/javascript">
    ...
    $(document).ready(function() {
        ...
        $("#btn_generate_proforma_invoice").click(function() {
            ...
            if (isValid1) {
                ...
                if ($("#seller_name").val() == 0) {
                    isValid4 = false;
                    $("#error-sellername").show();
                } else {
                    isValid4 = true;
                    $("#error-sellername").hide();
                    fd.append("sellerid", $("#seller_name").val());
                }
                ...
                if ($("#update_buyer_ledger").is(":checked") == true) {
                    fd.append("update_buyer_ledger", 1);
                } else {
                    fd.append("update_buyer_ledger", 0);
                }

                fd.append("invoice_id", $("#hdnInvoiceId").val());
                ...
                if (isValid2 && isValid3 && isValid4) {
                    ...
                    if (origin == 3) {
                        ...
                        if ($("#transferprice").is(":checked") == true) {
                            fd.append("transferprice", 1);
                            if ($("#transfer_price").val().length == 0) {
                                fd.append("transfer_price_cost", 0);
                            } else {
                                fd.append("transfer_price_cost", $("#transfer_price").val());
                            }
                        } else {
                            fd.append("transferprice", 0);
                            fd.append("transfer_price_cost", 0);
                        }
        ...
        $("#seller_name").change(function() {
            $("#error-sellername").hide();
            if ($("#seller_name").val() == 0) {
                $("#seller_name_details").text("");
            } else {
                fetchSellerDetails($("#seller_name").val());
            }
        });
        ...
        $("#transferprice").change(function() {
            if ($("#transferprice").is(":checked") == true) {
                $("#transfer_price").removeAttr("disabled");
            } else {
                $("#transfer_price").attr("disabled", true);
                $("#transfer_price").val("");
            }
        });
    });
    ...
    function fetchSellerDetails(sellerid) {
        ...
        $("#loading").show();
        $.ajax({
            url: base_url + "/fetch_seller_details",
            cache: false,
            method: "POST",
            data: fd,
            contentType: false,
            ...
            success: function(JSON) {
                ...
                if (JSON.redirect == true) {
                    ...
                    toastr.clear();
                    $("#seller_name_details").text(JSON.result);
                } else {
                    toastr.clear();
                    toastr.error(JSON.error);
                }
