...
<?php echo form_open_multipart('costingpurchasers/add', $attributes, $hidden); ?>
<div class="modal-body supplier-modal">
	...
	<div class="row mb-3">
		<div class="col-md-6">
			<label for="purchaser_name"><?php echo $this->lang->line('purchaser_name'); ?></label>
			<input class="form-control" placeholder="<?php echo $this->lang->line('purchaser_name'); ?>" name="name" id="name" type="text" value="<?php echo isset($get_supplier_details[0]->purchaser_name) ? $get_supplier_details[0]->purchaser_name : ''; ?>">
			<label id="error-name" class="error-text"><?php echo $this->lang->line('error_name'); ?></label>
		</div>
		<div class="col-md-6">
			<label for="company_id"><?php echo $this->lang->line('company_id'); ?></label>
			<input class="form-control" placeholder="<?php echo $this->lang->line('company_id'); ?>" name="company_id" id="company_id" type="text" value="<?php echo isset($get_supplier_details[0]->company_id) ? $get_supplier_details[0]->company_id : ''; ?>">
	...
	<div class="row mb-3">
		...
		<div class="col-md-6">
			<label for="origin"><?php echo $this->lang->line('costing_type'); ?></label>
			<select class="form-control" name="costing_type[]" id="costing_type" data-plugin="select_erp" multiple>
				<?php foreach ($costingtypes as $costingtype) { ?>
					<?php if (isset($get_supplier_details[0]->costing_type)) { ?>
						<option value="<?php echo $costingtype->id; ?>" <?php if (in_array($costingtype->id, $get_supplier_details[0]->costing_type)) : ?> selected="selected" <?php endif; ?>> <?php echo $this->lang->line($costingtype->translate_name); ?></option>
					<?php } else { ?>
						<option value="<?php echo $costingtype->id; ?>"> <?php echo $this->lang->line($costingtype->translate_name); ?></option>
					<?php } ?>
				<?php } ?>
			</select>
...
<script type="text/javascript">
	...
	$(document).ready(function() {
		...
		$("#add_supplier").submit(function(e) {
			...
			var supplierid = $("#company_id").val().trim();
			...
			if (isValid1 && isValid2 && isValid3) {
				...
				fd.append("export_type", costing_type);
				4
