...
<div class="card mb-3">
    <div class="card-header table-responsive">
        <div class="row flex-between-end">
            <div class="col-auto align-self-center">
                ...
                <h3> <?php echo $this->lang->line('supplier_ledger_header'); ?> </h3>
            </div>
            
            <div class="col-auto ms-auto">
				<button class="btn btn-info btn-md" title="<?php echo $this->lang->line('download_reports'); ?>" type="button" id="btn_all_suppliers">
					<span class="fas fa-download" data-fa-transform="shrink-3 down-2"></span><span class="ms-1"><?php echo $this->lang->line('download_reports'); ?></span></button>
			</div>
        </div>
    </div>
    <div class="card-body pt-5">
        <div class="mb-3 row">
            ...
            <label class="col-sm-2 col-form-label lbl-font" for="supplier_name_ledger"><?php echo $this->lang->line('supplier_name'); ?></label>
            <div class="col-sm-10">
                <select class="form-control" name="supplier_name_ledger" id="supplier_name_ledger" data-plugin="select_erp">
                    <option value="0"><?php echo $this->lang->line('select'); ?></option>
                </select>
                <div class="mb-4 row"></div>
            </div>
...
<script type="text/javascript">
    ...
    $(document).ready(function() {
        ...
        $("#origin_ledger").change(function() {
            ...
            fetchSuppliers($("#origin_ledger").val());
        });

        $("#supplier_name_ledger").change(function() {
        ...
        $("#btn_all_suppliers").click(function() {
                if($("#origin_ledger").val() == 0) {

                    toastr.clear();
                    toastr.warning(error_origin_screen);
                    ...
                    $("#loading").show();
                    $.ajax({
                        url: base_url + "/generate_all_supplier_ledger?originid=" + $("#origin_ledger").val(),
                        cache: false,
                        method: "GET",
                        dataType: "json",
        ...
        $("#btn_download_reports").click(function() {
            ...
            } else if ($("#supplier_name_ledger").val() == 0) {
    ...
    function fetchSuppliers(originid) {
        $("#loading").show();
        $.ajax({
            url: base_url + "/get_suppliers_by_origin?originid=" + originid,
            cache: false,
            method: "GET",
            dataType: "json",
            success: function(JSON) {
                ...
                if (JSON.redirect == true) {
                    ...
                    $("#supplier_name_ledger").empty();
                    $("#supplier_name_ledger").append(JSON.result);
                }
            }
        });
    }
