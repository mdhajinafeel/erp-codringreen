...
class Reception_model extends CI_Model
{
    ...
    {
        $query = $this->db->query("SELECT B.inventory_order, A.no_of_pieces, A.scanned_code, A.length, A.width, A.thickness, 
        ...
        foreach ($data as $r) {
            if ($purchaseunitid == 1) {
                ...
                $dataReception = array(
                    "reception_id" => $receptionid, "salvoconducto" => $inventorynumber,
                    "scanned_code" => $r->no_of_pieces, "length_bought" => $r->length, "width_bought" => 0, "thickness_bought" => 0,
                    "circumference_bought" => $r->circumference, "volumepie_bought" => 0, "cbm_bought" => $r->volume,
                    "length_export" => 0, "width_export" => 0, "thickness_export" => 0,
                    "cbm_export" => $r->volume, "grade" => 0, "face" => 0,
                    "createddate" => date('Y-m-d H:i:s'), "createdby" => $userid,
                    "updateddate" => date('Y-m-d H:i:s'), "updatedby" => $userid,
                    "isactive" => 1, "isdispatch" => 0, "scanned_timestamp" => 0, "isduplicatescanned" => 0, 
    ...
    public function get_reception_details($receptionid, $inventoryorder)
    {
        $query = $this->db->query("SELECT A.reception_id, A.supplier_id, C.supplier_name, C.supplier_code, A.salvoconducto, 
                        ...
                        getremainingvolume_reception(A.reception_id, A.salvoconducto) AS remaining_volume, A.isclosed, A.circ_allowance, A.length_allowance, A.rounding_factor  
    ...
    public function get_reception_lists($originid)
    {
        $query = $this->db->query("SELECT reception_id, salvoconducto, measurementsystem_id, origin_id, received_date, 
                ...
                is_special_uploaded, B.product_name, D.type_id 
                FROM tbl_reception A 
