...
class Reception_model extends CI_Model
{
    ...
    public function add_reception_data_from_farm($receptionid, $inventorynumber, $purchaseunitid, $userid)
    {
        $query = $this->db->query("SELECT B.inventory_order, A.scanned_code, A.length, A.width, A.thickness, 
                ...
                A.volume, A.grade_id FROM tbl_farm_data A 
        ...
        foreach ($data as $r) {
            if ($purchaseunitid == 1) {
                $dataReception = array(
                    ...
                    "cbm_export" => $r->volume, "grade" => $r->grade_id,
    ...
    public function get_reception_details($receptionid, $inventoryorder)
    {
        $query = $this->db->query("SELECT A.reception_id, A.supplier_id, C.supplier_name, C.supplier_code, A.salvoconducto, 
                        A.received_date, A.warehouse_id, B.warehouse_name, E.product_name, 
    ...
    public function get_warehouse_report_by_supplier($originid, $supplierid, $inventoryorder, $producttypeid)
    {
        if ($producttypeid == 1 || $producttypeid == 3) {
            ...
            $strQuery = "SELECT supplier_name, supplier_code, salvoconducto, product_name, product_type_name, 
                    ...
                    is_special FROM v_fetch_warehouse_report_data 
    ...
    public function get_warehouse_report_by_product($originid, $productid, $producttypeid)
    {
        if ($producttypeid == 1 || $producttypeid == 3) {
            $strQuery = "SELECT supplier_name, supplier_code, salvoconducto, product_name, product_type_name, 
                    warehouse_name, measurement_name, received_by, received_date, length_bought, width_bought, 
    ...
    public function get_warehouse_report_by_daterange($originid, $startdate, $enddate, $producttypeid)
    {
        if ($producttypeid == 1 || $producttypeid == 3) {
            $strQuery = "SELECT supplier_name, supplier_code, salvoconducto, product_name, product_type_name, 
                    warehouse_name, measurement_name, received_by, received_date, length_bought, width_bought, 
    ...
    public function get_warehouse_report_by_producttype_round_logs($originid, $producttypeid)
    {
    ...
    public function get_warehouse_report_by_inventory($originid, $inventoryorder, $producttypeid)
    {
        if ($producttypeid == 1 || $producttypeid == 3) {
            $strQuery = "SELECT supplier_name, supplier_code, salvoconducto, product_name, product_type_name, 
                    warehouse_name, measurement_name, received_by, received_date, length_bought, width_bought, 
    ...
    {
        $query = $this->db->query("SELECT C.supplier_name, A.salvoconducto, B.warehouse_name, E.product_name, 
