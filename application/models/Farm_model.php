...
class Farm_model extends CI_Model
{
    ...
    public function get_farm_data_details($farmid, $contractid, $inventoryorder)
    {
        $query = $this->db->query("SELECT no_of_pieces AS no_of_pieces, A.circumference, A.length, A.scanned_code, A.width, A.thickness, 
                ...
                ORDER BY CASE WHEN (B.product_type_id = 1 OR B.product_type_id = 3) THEN A.scanned_code ELSE A.circumference END");
        return $query->result();
    }
    ...
    public function get_farm_report_by_daterange($originid, $startdate, $enddate, $producttypeid)
    {
        ...
        if ($producttypeid == 1 || $producttypeid == 3) {
            $strQuery = "SELECT supplier_name, supplier_code, product_name, product_type_name, scanned_code, 
                    ...
                    received_by, received_date, inventory_order, purchase_unit_id, 
                    circumference_allowance, length_allowance, contract_code
                    FROM v_fetch_farm_report_data WHERE (STR_TO_DATE(received_date, '%d/%m/%Y') BETWEEN STR_TO_DATE('$startdate', '%d/%m/%Y')  AND STR_TO_DATE('$enddate', '%d/%m/%Y')) AND origin_id = $originid AND product_type_id IN (1,3)";
        } else if ($producttypeid == 2 || $producttypeid == 4) {
            $strQuery = "SELECT supplier_name, supplier_code, product_name, product_type_name, scanned_code, 
                    no_of_pieces, circumference, length, received_by, received_date, inventory_order, purchase_unit_id, 
                    circumference_allowance, length_allowance, contract_code 
                    FROM v_fetch_farm_report_data WHERE (STR_TO_DATE(received_date, '%d/%m/%Y') BETWEEN STR_TO_DATE('$startdate', '%d/%m/%Y')  AND STR_TO_DATE('$enddate', '%d/%m/%Y')) AND origin_id = $originid AND product_type_id IN (2,4)";
        }
        $query = $this->db->query($strQuery);
        return $query->result();
    }
    ...
    public function fetch_farm_details($originid)
    {
    ...
    public function update_farm_data($farmdataid, $farmId, $data)
    {
        $multiClause = array('farm_data_id' => $farmdataid, "farm_id" => $farmId, 'is_active' => 1);
        $this->db->where($multiClause);
        $this->db->set('updated_date', 'NOW()', FALSE);
    ...
    public function get_farm_data_by_farm_id_and_length($farmid, $length)
    {
        if ($length == 1) {
    ...
    public function get_price_for_circumference_length($circumference, $purchasecontractid, $length)
    {
        if ($circumference == -1) {
            ...
            if ($length < 330) {
                $query = $this->db->query("SELECT pricerange_grade3 AS price
                    FROM tbl_supplier_contract_price WHERE is_active = 1 
                    AND $circumference BETWEEN minrange_grade1 AND maxrange_grade2 
                    AND supplier_id = $purchasecontractid");
            } else if ($length >= 330 && $length < 600) {
                $query = $this->db->query("SELECT pricerange_grade_semi AS price  
                    FROM tbl_supplier_contract_price WHERE is_active = 1 
                    AND $circumference BETWEEN minrange_grade1 AND maxrange_grade2 
                    AND supplier_id = $purchasecontractid");
            } else {
                $query = $this->db->query("SELECT pricerange_grade_longs AS price 
                    FROM tbl_supplier_contract_price WHERE is_active = 1 
                    AND $circumference BETWEEN minrange_grade1 AND maxrange_grade2 
                    AND supplier_id = $purchasecontractid");
            }
    ...
    public function fetch_farm_details_consolidated($originid, $supplierid, $fromdate, $todate, $producttypeid) {
        ...
        $strQuery = "SELECT LEFT(A.inventory_order, 11) AS base_number, DATE_FORMAT(A.purchase_date, '%d-%m-%Y') AS purchase_date, A.purchase_unit_id, C.description AS purchase_unit, 
            B.supplier_name, GROUP_CONCAT(A.inventory_order ORDER BY inventory_order) AS full_variants 
            FROM tbl_farm A
            INNER JOIN tbl_suppliers B ON B.id = A.supplier_id 
            INNER JOIN tbl_purchase_unit C ON C.id = A.purchase_unit_id 
        ...
        if($fromdate != '' && $todate != '') {
            $strQuery .= " AND A.purchase_date BETWEEN '$fromdate' AND '$todate'";
        }

        $strQuery .= " AND A.product_type_id IN ($producttypeids) GROUP BY base_number, supplier_name, purchase_unit_id ORDER BY A.purchase_date ASC";

        $query = $this->db->query($strQuery);
        return $query->result();
    }

    public function get_farm_data_by_inventory_order($inventoryorder) {
        $strQuery = "SELECT A.circumference, A.length, A.no_of_pieces, C.purchase_allowance, C.purchase_allowance_length FROM tbl_farm_data A 
                INNER JOIN tbl_farm B ON B.farm_id = A.farm_id 
                INNER JOIN tbl_supplier_purchase_contract C ON C.contract_id = B.contract_id 
                WHERE A.is_active = 1 AND B.inventory_order LIKE '%$inventoryorder%'";
        
        $query = $this->db->query($strQuery);
        return $query->result();
    }

    public function get_farm_details_byid_supplier($inventoryorder, $supplierid)
    {
        $query = $this->db->query("SELECT A.farm_id, A.inventory_order, A.supplier_id, B.supplier_name, B.supplier_code, C.contract_code, 
                D.product_name, E.product_type_name, DATE_FORMAT(A.purchase_date, '%d/%m/%Y') AS purchase_date, 
                A.service_cost, A.pay_service_to, CASE WHEN getsuppliername_bysupplierid(A.pay_service_to) IS NULL THEN '-' ELSE getsuppliername_bysupplierid(A.pay_service_to) END as service_pay_to, 
                A.logistic_cost, A.pay_logistics_to, CASE WHEN getsuppliername_bysupplierid(A.pay_logistics_to) IS NULL THEN '-' ELSE getsuppliername_bysupplierid(A.pay_logistics_to) END as logistic_pay_to, 
                A.wood_value, A.adjustment, gettotal_payamount_inventoryorder(A.inventory_order, A.contract_id) AS total_payment, 
                ...
                INNER JOIN tbl_product_types E ON E.type_id = A.product_type_id 
                INNER JOIN tbl_purchase_unit F ON F.id = C.unit_of_purchase
                WHERE A.is_active = 1 AND A.inventory_order = '$inventoryorder' 
