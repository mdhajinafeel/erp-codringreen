...
class Master_model extends CI_Model
{
	...
	{
		$query = $this->db->query("SELECT id, origin_name FROM tbl_origins WHERE is_active = 1 AND is_enabled_app = 1");
		return $query->result();
	}
	...
	{
		$query = $this->db->query("SELECT B.supplier_code, A.product_id AS product_name, A.product_name AS product_id_dispatch, 
	...
	{
		if($originid == 4) {
			$query = $this->db->query("SELECT whid, CONCAT(warehouse_name, ', ', B.origin_iso2_code) AS warehouse_name 
				FROM tbl_warehouses A 
				INNER JOIN tbl_origins B ON B.id = A.origin_id
				WHERE A.is_active = 1");
		} else {
	...
	//BUYERS
// 	public function fetch_buyers_list($originid)
// 	{
// 	    if($originid > 0) {
// 	        $originid = 0;
// 	    }
// 		$query = $this->db->query("SELECT id, buyer_name FROM tbl_master_buyers WHERE is_active = 1 AND origin_id = $originid");
// 		return $query->result();
// 	}
	...
	{
		if($originid == 4) {
			$query = $this->db->query("SELECT id, buyer_name FROM tbl_master_buyers WHERE is_active = 1 AND is_trader = 0");
		} else {
			if($originid > 0) {
				$originid = 0;
			}
	...
	public function get_buyers_details_by_id($buyerid)
	{
		$query = $this->db->query("SELECT UPPER(SPLIT_STR(buyer_name, ' ', 1)) AS invoice_name, buyer_name, address, contact_name, contact_no, email 
				FROM tbl_master_buyers WHERE is_active = 1 AND id = $buyerid");
		return $query->result();
	}

	//BANKS
	public function fetch_banks_invoice_list($originid)
	{
		$query = $this->db->query("SELECT id, bank_name FROM tbl_master_invoice_banks WHERE is_active = 1 AND origin_id = $originid ORDER BY origin_id ASC");
		return $query->result();
	}

	public function get_bank_details_by_id($bankid)
	{
		$query = $this->db->query("SELECT bank_name, bank_address, swift_code, account_number, pay_to  
				FROM tbl_master_invoice_banks WHERE is_active = 1 AND id = $bankid");
		return $query->result();
	}
	
	//SELLERS
	public function fetch_sellers_list()
	{
		$query = $this->db->query("SELECT id, seller_name FROM tbl_master_sellers WHERE is_active = 1");
		return $query->result();
	}

	public function get_seller_details_by_id($sellerid)
	{
		$query = $this->db->query("SELECT seller_name, address, contact_name, contact_no, email, template_file 
