...
// 				BETWEEN '$startdate' AND '$enddate')) AND A.product_type_id IN ($producttypeid) AND A.origin_id = $originid GROUP BY B.dispatch_id");
				...
				if($originid == 1) {
					...
					$query = $this->db->query("SELECT A.sa_number, CASE WHEN (A.shipped_date IS NULL or A.shipped_date = '') THEN '' ELSE DATE_FORMAT(STR_TO_DATE(A.shipped_date, '%d/%m/%Y'),'%M') END AS shipped_date, B.total_pieces, 
        				...
        				INNER JOIN tbl_export_container B ON B.container_details_id = A.id
        				INNER JOIN tbl_dispatch_container C ON C.dispatch_id = B.dispatch_id AND C.isactive = 1 
                        LEFT JOIN tbl_export_document_container D ON D.dispatch_id = C.dispatch_id AND D.export_type = 1 
                        LEFT JOIN tbl_export_documents D1 ON D1.export_id = A.id AND D1.export_type = 1 
                        LEFT JOIN tbl_export_document_container E ON E.dispatch_id = C.dispatch_id AND E.export_type = 2 
                        LEFT JOIN tbl_export_documents E1 ON E1.export_id = A.id AND E1.export_type = 2
                        LEFT JOIN tbl_export_document_container F ON F.dispatch_id = C.dispatch_id AND F.export_type = 3
                        LEFT JOIN tbl_export_documents F1 ON F1.export_id = A.id AND F1.export_type = 3
                        LEFT JOIN tbl_export_document_container G ON G.dispatch_id = C.dispatch_id AND G.export_type = 9
                        LEFT JOIN tbl_export_documents G1 ON G1.export_id = A.id AND G1.export_type = 9
                        LEFT JOIN tbl_export_document_container H ON H.dispatch_id = C.dispatch_id AND H.export_type = 4
                        LEFT JOIN tbl_export_documents H1 ON H1.export_id = A.id AND H1.export_type = 4
                        LEFT JOIN tbl_export_document_container I ON I.dispatch_id = C.dispatch_id AND I.export_type = 5
                        LEFT JOIN tbl_export_documents I1 ON I1.export_id = A.id AND I1.export_type = 5
                        LEFT JOIN tbl_export_document_container J ON J.dispatch_id = C.dispatch_id AND J.export_type = 6
                        LEFT JOIN tbl_export_documents J1 ON J1.export_id = A.id AND J1.export_type = 6
                        LEFT JOIN tbl_export_document_container K ON K.dispatch_id = C.dispatch_id AND K.export_type = 7
                        LEFT JOIN tbl_export_documents K1 ON K1.export_id = A.id AND K1.export_type = 7 
                        LEFT JOIN tbl_export_document_container L ON L.dispatch_id = C.dispatch_id AND L.export_type = 8
                        LEFT JOIN tbl_export_documents L1 ON L1.export_id = A.id AND L1.export_type = 8
        				AND C.isduplicatedispatched = 0
        				WHERE A.isactive = 1 AND B.isactive = 1 
        				AND (A.shipped_date = '' OR (STR_TO_DATE(A.shipped_date, '%d/%m/%Y')
        				BETWEEN '$startdate' AND '$enddate')) AND A.product_type_id IN ($producttypeid) AND A.origin_id = $originid GROUP BY B.dispatch_id ORDER BY A.sa_number ASC");
