...
class Financemaster_model extends CI_Model
{
	...
	public function get_farm_detail($contractid, $supplierid, $originid, $inventoryorder, $langcode)
	{
		...
		if($supplierid == 0) {
			$query = $this->db->query("SELECT A.farm_id, DATE_FORMAT(A.purchase_date, '%d %M %Y') AS purchase_date, A.inventory_order, 
						...
						A.logistic_cost, A.service_cost, A.adjustment, A.purchase_unit_id, A.exchange_rate, C.invoice_number    
						FROM tbl_farm A 
						INNER JOIN tbl_suppliers B ON B.id = A.supplier_id 
						INNER JOIN tbl_contract_inventory_mapping C ON C.contract_id = A.contract_id AND C.supplier_id = A.supplier_id AND C.inventory_order = A.inventory_order 
						WHERE A.contract_id = $contractid AND A.origin_id = $originid 
						AND A.inventory_order = '$inventoryorder' AND A.is_active = 1");
		} else {
			$query = $this->db->query("SELECT A.farm_id, DATE_FORMAT(A.purchase_date, '%d %M %Y') AS purchase_date, A.inventory_order, 
						A.plate_number, B.supplier_name, A.supplier_taxes_array, A.logistics_taxes_array, A.service_taxes_array, 
						A.logistic_cost, A.service_cost, A.adjustment, A.purchase_unit_id, A.exchange_rate, C.invoice_number    
	...
	public function get_farm_data($supplierid, $inventoryorder)
	{
		$query = $this->db->query("SELECT A.circumference_bought, A.length_bought, 
				...
				((A.isduplicatecaptured = 0) OR ISNULL(A.isduplicatecaptured))) AND A.origin_id = 1 
				AND A.salvoconducto = '$inventoryorder' AND A.supplier_id = $supplierid GROUP BY B.circumference_bought,A.salvoconducto
				UNION ALL
				...
				WHERE (A.is_active = 1) AND A.origin_id = 1 AND A.inventory_order = '$inventoryorder' AND A.supplier_id = $supplierid 
				GROUP BY B.circumference,A.inventory_order) x ORDER BY circumference_bought) A 
	...
	//COST SUMMARY REPORT
	public function get_export_sa_numbers()
	{
		$query = $this->db->query("SELECT id, sa_number FROM tbl_export_container_details 
				WHERE isactive = 1 AND origin_id = 1 ORDER BY id ASC");
		return $query->result();
	}
