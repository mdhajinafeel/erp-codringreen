...
class Login_model extends CI_Model
{
	...
	{
		$sql = "SELECT B.userid, B.fullname, B.profilephoto, 
				A.roleid, E.timezone_abbreviation AS timezone, F.language_name, B.contactno, B.emailid, B.address, C.rolename, 
				geticon_by_origin($originId) AS icon, getapplicableorigins_byid($originId) AS origin_name,
				H.currency_name, H.currency_abbreviation AS currency_format, H.currency_excel_format AS currency_format_excel 
				FROM tbl_login A 
				INNER JOIN tbl_user_registration B ON B.userid = A.userid 
				INNER JOIN tbl_origin_timezones D ON D.id = B.default_timezone 
                INNER JOIN tbl_master_timezones E ON E.id = D.timezone_id 
				INNER JOIN tbl_languages F ON F.id = B.default_language 
				INNER JOIN tbl_role_master C ON C.roleid = A.roleid 
