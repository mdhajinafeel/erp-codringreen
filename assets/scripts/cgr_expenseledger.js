$(document).ready(function () {
	...
	$(document).on('click', 'button[data-role=deletedebittransaction]', function () {
		var transaction_id = $(this).data('transaction_id');
		var transaction_display_id = $(this).data('transaction_display_id');
		var origin_id = $("#origin_ledger").val();
		var user_id = $("#beneficiary_name_ledger").val();
		...
		$.ajax({
			url: base_url + "/dialog_expense_action",
			type: "GET",
			data: 'jd=1&is_ajax=3&mode=modal&type=deletedebitconfirmation&tid=' + transaction_id + '&did=' + transaction_display_id + '&oid=' + origin_id + '&uid=' + user_id,
			success: function (response) {
				$("#loading").hide();
				if (response.redirect == true) {
					window.location.replace(login_url);
